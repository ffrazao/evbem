...
public class PessoaArquivo extends EntidadeBase implements Ordenavel, Priorizavel, Visivel {
	...
	public PessoaArquivo(Integer valor) {
		super(valor);
	}

	@Override
	public PessoaArquivo infoBasica() {
		PessoaArquivo result = (PessoaArquivo) super.infoBasica();
		if (result.getPessoa() != null) {
			result.setPessoa(new Pessoa(result.getPessoa().getId()));
		}
		if (result.getArquivo() != null) {
			result.setArquivo(result.getArquivo().infoBasica());
		}
		return result;
	}
